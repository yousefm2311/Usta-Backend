...
const crev = require('../../controllers/customer/customer.reviews.controller');
const fav = require('../../controllers/customer/customer.favorites.controller');
const cnot = require('../../controllers/customer/customer.notifications.controller');
const cans = require('../../controllers/customer/customer.analytics.controller');
const pay = require('../../controllers/customer/customer.payments.controller');
...
router.post('/api/customer/notifications/fcm-token', auth('customer'), body('token').isString().isLength({ min: 10 }), body('deviceId').isString().isLength({ min: 3 }), body('platform').optional().isIn(['android', 'ios', 'web']), handleValidation, (req, res, next) => notif.saveCustomerToken(req, res).catch(next));
router.get('/api/customer/notifications/fcm-token', auth('customer'), (req, res, next) => notif.listCustomerTokens(req, res).catch(next));
router.post('/api/customer/notifications/subscribe-topic', auth('customer'), body('topic').isString().matches(/^[a-z0-9_]+$/), body('deviceId').optional().isString().isLength({ min: 3 }), handleValidation, (req, res, next) => notif.subscribeCustomerTopic(req, res).catch(next));
router.post('/api/customer/notifications/unsubscribe-topic', auth('customer'), body('topic').isString().matches(/^[a-z0-9_]+$/), body('deviceId').optional().isString().isLength({ min: 3 }), handleValidation, (req, res, next) => notif.unsubscribeCustomerTopic(req, res).catch(next));
...
// Marketing & Extras
router.get('/api/customer/coupons', (req, res, next) => require('../../controllers/customer/customer.marketing.controller').coupons(req, res).catch(next));
router.post('/api/customer/coupons/apply', auth('customer'), body('code').isString().isLength({ min: 2 }), handleValidation, (req, res, next) => require('../../controllers/customer/customer.marketing.controller').applyCoupon(req, res).catch(next));
router.post('/api/customer/referral', auth('customer'), body('code').isString().isLength({ min: 3 }), handleValidation, (req, res, next) => require('../../controllers/customer/customer.marketing.controller').referral(req, res).catch(next));
router.get('/api/customer/rewards', auth('customer'), (req, res, next) => require('../../controllers/customer/customer.marketing.controller').rewards(req, res).catch(next));
router.get('/api/customer/recommendations', auth('customer'), (req, res, next) => require('../../controllers/customer/customer.marketing.controller').recommendations(req, res).catch(next));
router.get('/api/customer/live-map', auth('customer'), (req, res, next) => require('../../controllers/customer/customer.marketing.controller').liveMap(req, res).catch(next));
