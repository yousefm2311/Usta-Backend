...
router.post('/api/artisan/requests/:id/accept', auth('artisan'), param('id').isLength({ min: 24, max: 24 }), ok, (req, res, next) => ctrl.acceptRequest(req, res).catch(next));
router.post('/api/artisan/requests/:id/reject', auth('artisan'), param('id').isLength({ min: 24, max: 24 }), ok, (req, res, next) => ctrl.rejectRequest(req, res).catch(next));
