...
- `PUT /api/admin/requests/:id/cancel` — body `{ reason?, note? }`
- `POST /api/admin/requests/:id/timeline` — body `{ status, note? }`
- `GET /api/admin/requests/:id/messages`
- `POST /api/admin/requests/:id/messages` — body `{ message, attachments? }`
...
- Errors (موحد لكل المسارات): `{ error, message, code, details?, path, method, timestamp, stack? (dev only) }`
...
- `POST /api/admin/orders/:id/timeline` — body `{ status, note? }`
- `PUT /api/admin/orders/:id/cancel` — body `{ reason?, note? }`
- `PUT /api/admin/orders/:id/close` — body `{ note? }`
- `GET /api/admin/orders/:id/messages`
- `POST /api/admin/orders/:id/messages` — body `{ message, attachments? }`
